			 ...
			 IDirect3DDevice9** m_Device)// [out]The created m_Device.
{
	...
	// Step 4: Create the m_Device.

	hr = d3d9->CreateDevice(
		D3DADAPTER_DEFAULT, // primary adapter
		m_DeviceType,         // m_Device type
		hwnd,               // window associated with m_Device
		vp,                 // vertex processing
		&d3dpp,             // present parameters
		m_Device);            // return created m_Device
	...
	if( FAILED(hr) )
	{
		...
		if( FAILED(hr) )
		{
			...
			::MessageBox(0, "Createm_Device() - FAILED", 0, 0);
			return false;
		}
	}
...
bool d3d::BoundingBox::isPointInside(D3DXVECTOR3& p )
{
	...
	{
		if (_max.x >= p.x && _max.y >= p.y && _max.z >= p.z)
		{
			return true;
		} 
